package dev.xnative.cleanrmapi.characters.data.repositories
...
import dev.xnative.cleanrmapi.characters.data.extensions.toDetailedModel
...
internal class CharacterRepositoryImpl(
    ...
    override suspend fun getCharacters(): Flow<List<CharacterPreview>> =
        characterDAO
            .getCharacters()
            .mapElement(transform = CharacterObject::toPreview)
    ...
    override suspend fun getEpisodesWhere(characterId: Int): List<EpisodePreview> {
    ...
    private suspend fun getEpisodesFromIdList(@MustBeCommaSeparatedIds idList: String): List<EpisodePreview> {

        return if (idList.contains(",")) {
            val episodesResponse = episodeApi.getEpisodesFromIds(ids = idList)
            episodesResponse.map { it.toDBObject().toPreview() }
        } else {
            val episodeResponse = episodeApi.getEpisode(idList.toInt())
            episodeResponse?.toDBObject()?.toPreview()?.toList() ?: emptyList()
        }
